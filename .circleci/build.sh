...
git clone --depth=1 -b nethrebased https://github.com/prorooter007/Nethunter_Kernel_Xiaomi_msm8953 kernel
cd kernel
...
# Compile plox
function compile() {
    make -j$(nproc) O=out ARCH=arm64 nethunter-tissot_defconfig
    make -j$(nproc) O=out \
                    ARCH=arm64 \
                      CC=clang \
